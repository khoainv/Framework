    ...
    elFinder = function (el, o) {
        ...
        */
        ...
        this.select = function (el, reset) {
            ...
            self.updateSelect();
            //Check is image to show image preview
            if ($('div#image_property').html() == '') {
                $('div#image_property').html($('div#panel_preview').html());
                $('div#panel_preview').html('');
            }

            if ($('li[name*="resize"]').hasClass("disabled") == false) {

                $('div#panel_wrapper_detail').css("display", "block");
                var i, s = [];
                for (i = 0; i < this.selected.length; i++) {
                    this.cdc[this.selected[i]] && s.push(this.cdc[this.selected[i]]);
                ...
                if (s[0]) {
                    ...
                    $('div#general_panel input#src').val(s[0].url);
                    $('div#general_panel input#alt').val(s[0].name);
                    var wth = $('div#general_panel input#width');
                    var hth = $('div#general_panel input#height');
                    wth.val(s[0].dim.split('x')[0]);
                    hth.val(s[0].dim.split('x')[1]);
                    $('div#general_panel input#tmp_width').val(s[0].dim.split('x')[0]);
                    $('div#general_panel input#tmp_height').val(s[0].dim.split('x')[1]);
                    wth.add(hth).bind('change', setDimensions);
                    
                }
            }
            else
                $('div#panel_wrapper_detail').css("display", "none");

            function setDimensions() {
                if (!$('input#constrain').is(':checked'))
                    return;
                var rel = parseInt($('input#tmp_width').val()) / parseInt($('input#tmp_height').val());

                var _w = parseInt($('input#width').val()) || 0;
                var _h = parseInt($('input#height').val()) || 0;

                if (_w <= 0 || _h <= 0) {
                    _w = parseInt($('input#tmp_width').val());
                    _h = parseInt($('input#tmp_height').val());
                } else if (this == $('input#width').get(0)) {
                    _h = parseInt(_w / rel);
                } else {
                    _w = parseInt(_h * rel);
                }
